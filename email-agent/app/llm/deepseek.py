...
class DeepSeekProvider(BaseLLMProvider):
    ...
    def __init__(
        ...
        if model not in self.MODELS:
            logger.warning(f"未知的DeepSeek模型: {model}, 使用默认模型")
    ...
    async def chat(
        ...
        **kwargs
        ...
        logger.debug(f"DeepSeek请求: model={use_model}, messages_count={len(messages)}")
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if not choices:
                    raise Exception("DeepSeek返回空响应")

                content = choices[0].get("message", {}).get("content", "")
                finish_reason = choices[0].get("finish_reason", "stop")
                usage = data.get("usage", {})

                logger.debug(f"DeepSeek响应: content_length={len(content)}, usage={usage}")
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"DeepSeek连接验证失败: {e}")
            return False